...
function OpenArmoryMenu(station)
	...
	if Config.EnableArmoryManagement then
		table.insert(elements, {label = _U('delete_weapon'),     value = 'put_weapon'})
        -- table.insert(elements, {label = _U('delete_weapon'),  value = 'delete_weapon'})
		table.insert(elements, {label = _U('deposit_object'), value = 'put_stock'})
		table.insert(elements, {label = _U('item_shop'), value = 'item_shop'})
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory', {
		...
		elseif data.current.value == 'put_stock' then
			OpenPutStocksMenu()
		elseif data.current.value == 'item_shop' then
			OpenGetStocksMenu()
		end
...
RegisterNetEvent("esx_policejob-client-Bekijkinbeslagname")
AddEventHandler("esx_policejob-client-Bekijkinbeslagname", function()
    ESX.TriggerServerCallback("esx_policejob:server:Voertuigeninbeslag", function(data)
        local inv = {}
...
ShowCarInfo = function(x)
    ESX.TriggerServerCallback("esx_policejob:server:getvehowner", function(name)
        ESX.UI.Menu.CloseAll()
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'warehouse_laptop', {
            ...
            elements = {
                ...
                {label = "Voertuig Terug geven", functie = "esx_policejob:server:givecarback", info = x},
        }}, function(data, menu)
            if data.current.functie ~= "niks" then
                local v = Config.InbeslagnameVoertuigspawn
                if ESX.Game.IsSpawnPointClear(vector3(v.x,v.y,v.z), 5) then
                    ...
                    ESX.Game.SpawnVehicle(x.model, vector3(v.x,v.y,v.z), v.h, function(veh)
						...
						TriggerServerEvent('esx_vehiclelock:server:AddOwner', props.plate)
                    end)
                    menu.close()
                else
                    ESX.ShowNotification("De locatie is gehindert")
                end
            end
        end, function(data, menu)
            menu.close()
...
function OpenpoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions',
	{
		...
		if data.current.value == 'jail_menu' then
            TriggerEvent("esx_jail:client:open:menu")
		end
		-- esx_qalle_jail-end
			...
			if #playersNearby > 0 then
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_selection', {
					...
					local elements = {
						...
						{label = "Boete Menu",			value = 'fine'},
						{label = _U('fine'),			value = 'boeteopleggen'},
						...
						'default', GetCurrentResourceName(), 'citizen_interaction',
						{
							...
							if closestPlayer ~= -1 and #(GetEntityCoords(GetPlayerPed(selectedPlayer)) - coords) <= 10.0 then
								...
								elseif action == 'body_search' then
									-- TriggerServerEvent('plj:msg', serverPlayer, _U('being_searched', GetPlayerServerId(PlayerId())), `esx_policejob:message`)
								...
								elseif action == 'handcuff' then
									...
									Citizen.Wait(1000)																									-- Czeka 3.1 Sekund**
									TriggerServerEvent('esx_policejob:handcuff', serverPlayer, `esx_policejob:handcuff`)
								elseif action == 'uncuff' then
									...
									TriggerServerEvent('esx_policejob:handcuff', serverPlayer, false, `esx_policejob:handcuff`)
								...
								elseif action == 'put_in_vehicle' then
									TriggerServerEvent('esx_policejob:putInVehicle', serverPlayer, `esx_policejob:putInVehicle`)
								elseif action == 'out_the_vehicle' then
									TriggerServerEvent('esx_policejob:OutVehicle', serverPlayer, `esx_policejob:OutVehicle`)
								elseif action == 'fine' then
									OpenFineMenu(serverPlayer)
								elseif action == "boeteopleggen" then 
									ESX.UI.Menu.Open(
							'dialog', GetCurrentResourceName(), 'billing',
							{
								title = "Factuur aantal"
							},
								function(data, menu)
									local amount = tonumber(data.value)
										if amount == nil then
											ESX.ShowNotification("~r~Geen aantal aangegeven.")
										else
											menu.close()
											local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
												if closestPlayer == -1 or closestDistance > 3.0 then
													ESX.ShowNotification("~r~Geen inwoners in de buurt")
												else
													TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_police', "Politie Daily", amount)
												end
										end
								end,
							function(data, menu)
								...
								elseif action == 'revive' then
									if not IsBusy then
										...
										if exports['medSystem']:canRevive(serverPlayer) then
											...
											ESX.ShowNotification("Deze persoon is niet meer te redden...")
										end
		...
		elseif data.current.value == 'object_spawner' then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				elements = {
					...
					{label = 'Lint', 		model = 'oes_politielint', freeze = true},
					{label = 'Schotten', 		model = 'prop_snow_sign_road_06g', freeze = true},
					{label = 'Tent', 		model = 'oes_lijktent1', freeze = true},
			}}, function(data2, menu2)
				local playerPed = PlayerPedId()
	...
	ESX.TriggerServerCallback('esx_policejob:getFineList', function(fines)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
			...
			if Config.EnablePlayerManagement then
				TriggerServerEvent('esx_billing:sendBill', player, 'society_police', "Politie Daily", data.current.amount)
			else
				TriggerServerEvent('esx_billing:sendBill', player, 'society_police', "Politie Daily", data.current.amount)
			end
...
function OpenBuyWeaponsMenu()
	local elements = {}
	local playerPed = PlayerPedId()
	for k,v in ipairs(Config.AuthorizedWeapons[ESX.GetPlayerData().job.grade_name]) do
		local weaponNum1, weapon1 = v.weapon
		local dd = v.label
		local weaponNum, weapon = ESX.GetWeapon("WEAPON_"..v.weapon)
		...
		elseif hasWeapon and not v.components then
			label = ('%s: <span style="color:green;">%s</span>'):format(v.label, _U('armory_owned'))
		else
			if v.price > 0 then
				label = ('%s: <span style="color:green;">%s</span>'):format(v.label, _U('armory_item', ESX.Math.GroupDigits(v.price)))
			else
				label = ('%s: <span style="color:green;">%s</span>'):format(v.label, _U('armory_free'))
			end
		end
...
function cuffrequest(source, args, raw)
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'request_boei', {
        ...
        TriggerServerEvent("slang:send:request", data.current.id)
        menu.close()
    end, function(data, menu)
        menu.close()
    end)
...
RegisterCommand("request_cuff", cuffrequest)

RegisterNetEvent("slang:getrequest")
AddEventHandler("slang:getrequest", function(sender)
    being_asked(sender)
...
AddEventHandler("requestacceptedcuff", function(target)
...
being_asked = function(sender)
    Citizen.CreateThread(function()
        while true do 
            Wait(0)
            if IsControlJustReleased(0,246) then 
                TriggerServerEvent("acceptrequestboeien", sender)
            ...
            if IsControlJustReleased(0, 182) then 
                TriggerServerEvent("revokerequestboeien")
                ESX.ShowNotification("Verzoek op boeien afgewezen!")
                return
            end
        end
